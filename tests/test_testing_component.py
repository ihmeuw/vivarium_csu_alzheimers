...
class TestTestingForAlzheimers:
    ...
    def test_probability_positive_with_ad(self):
        ...
        # Create component
        component = TestingForAlzheimers(
            "alzheimers_pet_scan", "alzheimers_disease_and_other_dementias"
        )
        ...
        assert (
            prob_positive == sensitivity
        ).all(), (
            f"Expected all probabilities to be {sensitivity}, got {prob_positive.unique()}"
        )
    ...
    def test_probability_positive_without_ad(self):
        ...
        assert len(prob_positive) == 1000
        assert (
            prob_positive == expected_prob
        ).all(), (
            f"Expected all probabilities to be {expected_prob}, got {prob_positive.unique()}"
        )
    ...
    def test_probability_positive_mixed_population(self):
        ...
        ad_statuses = ["alzheimers_first_state"] * 500 + [
            "susceptible_to_alzheimers_disease_and_other_dementias"
        ] * 500
        pop_view_mock = Mock()
        pop_data = pd.DataFrame(
            {"alzheimers_disease_and_other_dementias": ad_statuses}, index=range(1000)
        )
        pop_view_mock.get.return_value = pop_data
        builder.population.get_view.return_value = pop_view_mock
        ...
        assert (
            with_ad_probs == sensitivity
        ).all(), f"Expected AD group to have probability {sensitivity}, got {with_ad_probs.unique()}"

        # Next 500 (without AD) should have (1-specificity) probability
        without_ad_probs = prob_positive[500:]
        expected_prob = 1 - specificity
        assert (
            without_ad_probs == expected_prob
        ).all(), f"Expected non-AD group to have probability {expected_prob}, got {without_ad_probs.unique()}"
    ...
    def test_probability_negative_is_complement(self):
        ...
        assert (
            prob_sum == 1.0
        ).all(), f"Probabilities should sum to 1.0, got {prob_sum.unique()}"
    ...
    def test_machine_integration(self):
        ...
        assert (
            ...
            f"AD group should have sensitivity {sensitivity}, got {with_ad_probs.unique()}"
        )
        ...
        assert (
            without_ad_probs == expected_prob
        ).all(), f"Non-AD group should have probability {expected_prob}, got {without_ad_probs.unique()}"
    ...
    def test_simulation_runs_successfully(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                result.returncode == 0
            ), f"Simulation failed with return code {result.returncode}. Error: {result.stderr}"

            # Check that expected output messages are present
            output = result.stdout + result.stderr
            assert (
                "Simulation finished" in output
            ), f"Simulation didn't finish properly. Output: {output[-500:]}"
            assert (
                "Results written to" in output
            ), f"Results weren't written. Output: {output[-500:]}"
